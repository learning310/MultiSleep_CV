...
def main():
    ...
    parser.add_argument(
        "--data_dir",
        type=str,
        default=r"D:\Data\Sleepedfx\sleep-cassette",
    ...
    parser.add_argument(
        "--select_ch", type=str, default="EEG Fpz-Cz", help="The selected channel"
    )
    ...
    for i in range(len(psg_fnames)):
        ...
        eeg = raw.to_data_frame(scaling_time=100.0)[select_ch]
        eeg = eeg.to_frame()
        eeg.set_index(np.arange(len(eeg)))

        eog = raw.to_data_frame(scaling_time=100.0)["EOG horizontal"]
        eog = eog.to_frame()
        eog.set_index(np.arange(len(eog)))
        ...
        eeg = eeg.values[select_idx]
        eog = eog.values[select_idx]

        # Verify that we can split into 30-s epochs
        if len(eeg) % (EPOCH_SEC_SIZE * sampling_rate) != 0:
            raise Exception("Something wrong")
        n_epochs = len(eeg) / (EPOCH_SEC_SIZE * sampling_rate)

        # Get epochs and their corresponding labels
        eeg = np.asarray(np.split(eeg, n_epochs)).astype(np.float32)
        eog = np.asarray(np.split(eog, n_epochs)).astype(np.float32)
        ...
        print("Data before selection: {}, {}".format(eeg.shape, y.shape))
        eeg = eeg[select_idx]
        eog = eog[select_idx]
        y = y[select_idx]
        print("Data after selection: {}, {}".format(eeg.shape, y.shape))
